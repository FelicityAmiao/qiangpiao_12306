...
class BrushTicket(object):
    ...
    def __init__(self, passengers, from_time, from_station, to_station, numbers, seat_type, receiver_mobile,
        ...
        # Disable dialogs
        self.disable_dialogs()

    def disable_dialogs(self):
        """Disable alert, confirm, and prompt dialogs"""
        disable_js = """
            window.alert = function() {};
            window.confirm = function() { return true; };
            window.prompt = function() { return null; };
            """
        self.driver.execute_script(disable_js)
    ...
    def send_mail(self, receiver_address, content):
        ...
        host = 'smtp.163.com'
        port = 25  # SSL port
        sender = 'amiaoabc@163.com'  # 你的发件邮箱号码
        pwd = 'CBupCFFeYtarbpVs'  # 不是登陆密码，是客户端授权密码
        # 发件信息
...
if __name__ == '__main__':
    st.title("高铁抢票表单界面")
    # 乘客姓名
    passengers_input = st.text_input("请输入乘车人姓名，多人用英文逗号“,”连接，（例如单人“张三”或者多人“张三,李四”，如果学生的话输入“王五()”）")
    ...
    # 座位类型
    seat_type = st.text_input('请输入座位类型（例如“软卧”）：')
    # 抢票成功，通知该手机号码
    ...
    receiver_email = st.text_input('请预留一个邮箱，方便抢到票后进行通知（例如：test@163.com）：')
    submit = st.button("点击跳转登录12306进行抢票")

    # validation
    if submit and passengers_input == '' or len(passengers) > 4:
        st.info('乘车人最少1位，最多4位！')
        st.stop()

    date_pattern = re.compile(r'^\d{4}-\d{2}-\d{2}$')
    if submit and from_time == '':
        st.info('乘车日期不能为空或者时间格式不正确，请重新输入')
        st.stop()

    if submit and from_input not in city_list.keys():
        from_input = st.text_input('出发站不能为空或不支持当前出发站（如有需要，请联系管理员！），请重新输入')

    if submit:
        from_station = city_list[from_input]

    if submit and to_input not in city_list.keys():
        to_input = st.text_input('终点站不能为空或不支持当前终点站（如有需要，请联系管理员！），请重新输入')

    if submit:
        to_station = city_list[to_input]

    if submit and number_input == '':
        number_input = st.text_input('请重新输入抢票车次号，多车次用英文逗号“,”链接，（例如单车次“Z285”或者多车次“Z285,G110”）')
        numbers = number_input.split(",")

    if submit and seat_type == '':
        seat_type = st.text_input('座位类型不能为空，请重新输入')

    if submit and receiver_email == '':
        receiver_email = st.text_input('预留邮箱不能为空，请重新输入：')
